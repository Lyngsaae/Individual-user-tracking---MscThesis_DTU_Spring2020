...
for video_index in range(5,6):
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            # Check if YOLO is needed
            yolo_flag = (not ad_confidence or YOLO_count < 2 and len(true_feature_buffer) > 5) and AD_count == 0 or signal_start or start_funcion and not lock_flag
            ...
            if not yolo_flag:
                ...
                # Check similarity - re-identify user
                condition, dst, min_dst, fake_dst, mean_dst = test_similarity(feature_vector, true_feature_buffer, false_feature_buffer, max((threshold,3.5)))

                # Udate true feature buffer
                if dst < threshold  and (lock_flag or not start_funcion) and condition:
                    true_feature_buffer.append(id_model(id_frame.unsqueeze(0)))
                    if len(true_feature_buffer) > 20:
                        true_feature_buffer.pop(2)
            ...
            if yolo_flag or not ad_confidence:
                ...
                if type(output) != int:
                    ...
                    for x in output:
                        # Ignore if bounding fox is to small
                        ...
                        # Prepare corp and run Siamese network
                        ...
                        # Re-identify user
                        condition, dst, min_dst, fake_dst, mean_dst = test_similarity(feature_vector, true_feature_buffer, false_feature_buffer, threshold)

                        # Store test results for later user
                        temp_fature_vectors.append(feature_vector)
                        mean_dst_buffer.append(mean_dst)
                        fake_dst_bufer.append(fake_dst)

                        # Detect starting signal
                        if start_funcion and (not lock_flag or pre_signal_start or len(true_feature_buffer) <= 5):
                            # Offset to contain the whole human while performing the start signal
                        ...
                        if not start_funcion and not signal_start and len(true_feature_buffer) <= 5:
                            AD_bb = AD_bb_temp
                            index = i
                            signal_start = True

                        # Look for reset signal and update most likey users feature vector
                        if len(true_feature_buffer) > 5 and (not signal_start or lock_flag):
                            ad_confidence = True
                    ...
                    # Update false feature buffer according to IoU
                    if lock_flag or not start_funcion:
                        # If Potential user is found, update Bounding boxes and feature vectors accordingly to IoU
                        if index != -1:
                            IoU_condition = True
                            c1 = AD_bb[:2]
                            ...
                            for x, i in zip(output, range(len(output))):
                                ...
                                if IoU > 0.05:
                                    ...
                                    false_feature_buffer.extend(temp_fature_vectors[i])
                        else:
                            # Measure IoU between all bounding boxes and update thereafter
                    ...
                    if index != -1 and len(true_feature_buffer) or signal_start and not lock_flag:
                        ad_confidence = True
                        if (min_dst_total < threshold or signal_start ) and (signal_counter > 3 or lock_flag or not start_funcion) and IoU_condition:
                            true_feature_buffer.append(temp_fature_vectors[index])
                            if len(true_feature_buffer) > 20:
                                true_feature_buffer.pop(2)
                    elif index == -1:
                        ad_confidence = False
            ...
            # Update re-identification threshold
            if lock_flag:
